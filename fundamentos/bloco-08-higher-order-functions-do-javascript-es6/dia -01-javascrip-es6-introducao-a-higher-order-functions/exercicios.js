...
// cada pessoa contratada em seu respectivo id . A sua função deve receber como parâmetro
// o nome completo da pessoa funcionária e a partir dele gerar automaticamente um 
// email no formato nome_da_pessoa@trybe.com .

//a) criar uma função que retorne { nomeCompleto, email }
//b)Passe sua função como parâmetro da HOF newEmployees para criar
// cada pessoa contratada em seu respectivo id 
//c)função deve receber como parâmetro o nome completo da pessoa funcionária
//d)gerar automaticamente um email no formato nome_da_pessoa@trybe.com 

const employeesPerson = (nomeCompleto) => {
...
console.log(newEmployees(employeesPerson));
...
//1 vou fazer um ternary.

const checkSorteio = (numeroApostado, numeroSorteado) => numeroApostado === numeroSorteado;

const resultSort = (numeroApostado, callback) => {
  const numeroSorteado =  Math.floor((Math.random() * 5) + 1);
  return callback(numeroSorteado, numeroApostado) ? `"Parabéns, você ganhou")`: `"Tente novamente"`;
};
console.log(resultSort(5, checkSorteio));
...
//final recebida pela pessoa estudante. Ao final a HOF deve retornar o total da contagem de respostas certas.
